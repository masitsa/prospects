		...
		if ($query->num_rows() > 0)
		{
			...
			foreach ($query->result() as $row)
			{
				...
				if($post_status == 0)
				{
					...
					$button = '<a class="btn btn-info btn-sm" href="'.site_url().'activate-post/'.$post_id.'" onclick="return confirm(\'Do you want to activate '.$post_title.'?\');">Activate</a>';
				}
				//create activated status display
				else if($post_status == 1)
				{
					$status = '<span class="label label-info">Active</span>';
					$button = '<a class="btn btn-default btn-sm" href="'.site_url().'deactivate-post/'.$post_id.'" onclick="return confirm(\'Do you want to deactivate '.$post_title.'?\');">Deactivate</a>';
				}
...
<section class="panel">
							...
							<div class="panel-body">
                            	...
                            	<div class="row" style="margin-bottom:20px;">
                                    <div class="col-lg-12">
                                    	<a href="<?php echo site_url();?>add-post" class="btn btn-success btn-sm pull-right">Add Post</a>
                                    </div>
                                </div>
